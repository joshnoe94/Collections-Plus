  ...
  const data = await mutate((data) => {
    ...
    if (col.parentId) {
      ...
      if (parent && !parent.items.find((i) => i.type === 'collection' && i.cid === col.id)) {
        addItem(col.parentId, {type: 'collection', cid: col.id})
      }
    }
  });

  // Recursively restore any subcollections
  const parent = findCollection(data, data.activeCollectionId);
  ...
  await mutate(async (data) => {
    ...
    for (const it of from.items) {
      ...
      if (it.type === 'collection') {
        // Copy all items in fromId's subcollection to a new subcollection of toId
        const fromChild = findCollection(data, it.cid);
        const toChild = await createSubCollection(toId, fromChild.title);
        await copyItems(fromChild.id, fromChild.items.map(item => item.id), toChild.id);
        continue;
      }
